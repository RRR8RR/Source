...
from Runthon import Convert, zedub
...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"Runthon_{userid}_{pack}_anim"
    if is_video:
        return f"Runthon_{userid}_{pack}_vid"
    return f"Runthon_{userid}_{pack}"
...
async def get_pack_info(event):
    ...
    OUTPUT = (
        f"𓆰 𝑺𝑶𝑼𝑹𝑪𝑬 𝐑𝐔𝐍𝐓𝐇𝐎𝐍  - 𝑺𝑻𝑰𝑪𝑲𝑹𝑺 𝑰𝑵𝑭𝑶 𓆪\n"
