...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝐑𝐔𝐍𝐓𝐇𝐎𝐍  - اوامــر المطـور المســاعد](t.me/xLxLxLrr3) 𓆪\n\n"
    ...
    "\n𓆩 [ 𝗠𝗢𝗛𝗔𝗠𝗠𝗔𝗗 - مطـور السـورس](t.me/PPF22) 𓆪"
)
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت رنثـون  𝐑𝐔𝐍𝐓𝐇𝐎𝐍  انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝐑𝐔𝐍𝐓𝐇𝐎𝐍  :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your Runthon__.\n"
                )
            else:
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت رنثـون  𝐑𝐔𝐍𝐓𝐇𝐎𝐍  انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
